var StakingEdit = React.createClass({
    ...
    onMainTokenInput(value) {
        if(!window.isEthereumAddress(value)) {
            return this.setState({mainTokenSymbol : "", mainTokenImage : ""});
        }
        var _this = this;
        window.loadTokenInfos(value, window.wethAddress).then(it => {
            _this.setState({mainTokenSymbol : it.symbol, mainTokenImage : it.logoURI});
        });
    },
    componentDidMount() {
        var _this = this;
        ...
        return (<section>
            ...
            <section className="TheDappInfo2">
                <section className="DFOTitleSection BravPicciot">
                    ...
                    <section>
                        <section>
                            <input type="text" onKeyUp={window.onTextChange} action="onMainTokenInput"/>
                        </section>
                        {props.mainTokenSymbol && <section>
                            <img src={props.mainTokenImage}/>
                            <span>{props.mainTokenSymbol}</span>
                        </section>}
                    </section>
